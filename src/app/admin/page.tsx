...
export default function AdminPage() {
    ...
    const [activeTab, setActiveTab] = useState<'projects' | 'realtors' | 'settings'>('projects');
    const [selectedRealtorIds, setSelectedRealtorIds] = useState<string[]>([]);

    // Settings State
    const [hideOurHomesPage, setHideOurHomesPage] = useState(false);
                ...
                <div className="max-w-2xl">
                    <div className="border border-blueprint-line p-8 bg-blueprint/30 backdrop-blur-sm relative">
                        ...
                        <div className="space-y-8 mt-4">
                            {/* Navigation Settings */}
                            <div>
                                <h3 className="text-white font-bold uppercase tracking-widest font-mono text-sm mb-4 border-b border-blueprint-line pb-2">Navigation</h3>
                                <div className="space-y-4">
                                    <label className="flex items-center justify-between gap-4 p-4 bg-black/20 border border-blueprint-line hover:border-blueprint-accent/50 transition-colors cursor-pointer group">
                                        <div>
                                            <span className="text-white font-mono text-sm block">Hide &quot;Our Homes&quot; Page</span>
                                            <span className="text-gray-500 text-xs mt-1 block">Removes the &quot;Our Homes&quot; link from the navigation header. Use this when you only have one home listed and the home page is sufficient.</span>
                                        </div>
                                        <div className="relative shrink-0">
                                            <input
                                                type="checkbox"
                                                checked={hideOurHomesPage}
                                                onChange={(e) => {
                                                    setHideOurHomesPage(e.target.checked);
                                                    updateSetting('hideOurHomesPage', e.target.checked);
                                                }}
                                                className="sr-only peer"
                                            />
                                            <div className="w-11 h-6 bg-black/40 border border-blueprint-line peer-checked:bg-blueprint-accent/30 peer-checked:border-blueprint-accent rounded-full transition-all" />
                                            <div className="absolute left-[2px] top-[2px] w-5 h-5 bg-gray-400 peer-checked:bg-blueprint-accent rounded-full transition-all peer-checked:translate-x-5" />
                                        </div>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                {settingsLoading && (
                                    <span className="text-blueprint-accent text-xs font-mono animate-pulse">Saving...</span>
                                )}
                                {settingsSaved && (
                                    <span className="text-green-400 text-xs font-mono">✓ Saved</span>
                                )}
                            </div>
                        </div>
                    </div>
